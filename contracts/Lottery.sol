...
contract Lottery is KeeperCompatibleInterface, VRFConsumerBase, ConfirmedOwner{
    ...
    function buyTicket(uint256 _noOfTickets) public payable inState(State.ACTIVE){
    ...
    function assignTickets(uint256 _noOfTickets) internal {
        ...
        uint256 newTotal = _lottery.noOfTicketsSold + _noOfTickets;

        for(uint n = oldTotal ; n < (newTotal-1); n++){
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        LotteryStruct storage _lottery = lotteries[lotteryID];
        _lottery.winningTicket = randomness % _lottery.noOfTicketsSold; 
    ...
    function payoutWinner() internal {
        ...
        uint256 reward = _lottery.amountInLottery / 2;
        require(IERC20(LISH).transferFrom(address(this), _lottery.winner, reward), "Transfer failed"); 
